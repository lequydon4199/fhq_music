...
class Search extends Component {
 ...
 _pressRow = (playlist) => {
  ...
  this.props.setSong(playlist.id, playlist.title, playlist.artist, playlist.artwork)
  ...
  searchedSong = (searchedText) => {
    var searchedSong = playlist.filter(function(playlist) {
      return playlist.title.toLowerCase().indexOf(searchedText.toLowerCase()) > 0;
    });
  ...
  renderSong = (playlist) => {
    return (
      <View style={styles.song}>
                    <TouchableOpacity
                      activeOpacity={0.5} onPress = {() => this._pressRow(playlist)}
                    >
                      <View style={styles.songContainer}>
                        <Image source ={{uri: playlist.artwork }}
